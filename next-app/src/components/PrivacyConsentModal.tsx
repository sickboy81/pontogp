  ...
  return (
    <div className="fixed inset-0 z-[110] flex items-center justify-center bg-slate-950/85 p-4 backdrop-blur-md" role="presentation">
      <section
        ...
        className="max-h-[calc(100vh-2rem)] w-full max-w-2xl overflow-y-auto rounded-3xl border border-slate-700 bg-slate-900 text-left shadow-2xl shadow-black/40"
      >
        {!settingsOpen ? (
          <div className="grid md:grid-cols-[210px_1fr]">
            ...
            <div className="p-6 sm:p-8">
              <div className="flex items-start gap-3">
                <ShieldCheck className="mt-0.5 h-6 w-6 shrink-0 text-primary-400" />
                <div>
                  <h3 id="consent-title" className="text-xl font-bold text-white sm:text-2xl">Antes de entrar</h3>
                  <p className="mt-3 text-sm leading-relaxed text-slate-300">
                    ...
                    <Link href="/termos" className="text-primary-300 hover:underline">Termos</Link> e{' '}
                    <Link href="/privacidade" className="text-primary-300 hover:underline">Política de Privacidade</Link>.
                  </p>
                </div>
              </div>
              <div className="mt-7 rounded-2xl border border-slate-700 bg-slate-950/40 p-4">
                <div className="flex items-center gap-3">
                  <Cookie className="h-5 w-5 text-primary-400" />
                  <h4 className="font-semibold text-white">Preferências de navegação</h4>
                </div>
                <p className="mt-2 text-sm leading-relaxed text-slate-400">Usamos cookies essenciais para segurança e login. Os opcionais ajudam a lembrar preferências e melhorar o serviço.</p>
              </div>
              <div className="mt-7 flex flex-col-reverse gap-3 sm:flex-row sm:items-center sm:justify-between">
                <button type="button" onClick={() => setSettingsOpen(true)} className="rounded-xl border border-slate-600 px-4 py-3 text-sm font-semibold text-slate-300 transition hover:border-slate-400 hover:text-white">Escolher cookies</button>
              ...
              <p className="mt-5 text-xs text-slate-500">Detalhes em <Link href="/cookies" className="text-primary-300 hover:underline">Aviso de Cookies</Link> e <Link href="/privacidade" className="text-primary-300 hover:underline">Privacidade</Link>.</p>
            </div>
          </div>
        ) : (
          <>
            <div className="p-6 sm:p-8">
            <h2 id="consent-title" className="text-xl font-bold text-white">Escolha seus cookies</h2>
            ...
            <div className="mt-6 space-y-3 text-left">
              <label className="flex items-center justify-between rounded-xl border border-slate-700 bg-slate-950/30 p-4">
                <span><strong className="block text-sm text-white">Necessários</strong><span className="text-xs text-slate-500">Sempre ativos</span></span>
                <Check className="h-5 w-5 text-primary-400" />
              </label>
              <label className="flex items-center justify-between rounded-xl border border-slate-700 bg-slate-950/30 p-4">
                <span><strong className="block text-sm text-white">Preferências</strong><span className="text-xs text-slate-500">Tema e escolhas de navegação</span></span>
                <input type="checkbox" checked={preferences} onChange={(e) => setPreferences(e.target.checked)} className="h-4 w-4 accent-primary-600" />
              </label>
              <label className="flex items-center justify-between rounded-xl border border-slate-700 bg-slate-950/30 p-4">
                <span><strong className="block text-sm text-white">Análise</strong><span className="text-xs text-slate-500">Medição de uso, quando habilitada</span></span>
            ...
            <button type="button" onClick={() => setSettingsOpen(false)} className="mt-4 text-sm text-slate-400 hover:text-white">
              Voltar
            </button>
            </div>
          </>
